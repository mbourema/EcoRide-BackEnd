 ...
 */
final class Version20250206080214 extends AbstractMigration
{
    ...
    public function up(Schema $schema): void
    {
        ...
        $this->addSql('CREATE TABLE covoiturage (covoiturage_id INT AUTO_INCREMENT NOT NULL, date_depart DATETIME NOT NULL, lieu_depart VARCHAR(100) NOT NULL, date_arrivee DATETIME NOT NULL, lieu_arrivee VARCHAR(100) NOT NULL, statut VARCHAR(255) NOT NULL, nb_places INT NOT NULL, prix_personne DOUBLE PRECISION NOT NULL, voiture_id INT NOT NULL, INDEX IDX_28C79E89181A8BA (voiture_id), PRIMARY KEY(covoiturage_id)) DEFAULT CHARACTER SET utf8mb4');
        $this->addSql('CREATE TABLE employe (employe_id INT AUTO_INCREMENT NOT NULL, PRIMARY KEY(employe_id)) DEFAULT CHARACTER SET utf8mb4');
        ...
        $this->addSql('CREATE TABLE paiement (paiement_id INT AUTO_INCREMENT NOT NULL, montant DOUBLE PRECISION NOT NULL, date_paiement DATETIME NOT NULL, avancement VARCHAR(255) NOT NULL, utilisateur_id INT NOT NULL, INDEX IDX_B1DC7A1EFB88E14F (utilisateur_id), PRIMARY KEY(paiement_id)) DEFAULT CHARACTER SET utf8mb4');
        ...
        $this->addSql('CREATE TABLE suspension (suspension_id INT AUTO_INCREMENT NOT NULL, raison VARCHAR(255) NOT NULL, date_debut DATETIME NOT NULL, date_fin DATETIME DEFAULT NULL, sanction VARCHAR(255) NOT NULL, utilisateur_id INT NOT NULL, employe_id INT NOT NULL, INDEX IDX_82AF0500FB88E14F (utilisateur_id), INDEX IDX_82AF05001B65292 (employe_id), PRIMARY KEY(suspension_id)) DEFAULT CHARACTER SET utf8mb4');
        $this->addSql('CREATE TABLE utilisateur (utilisateur_id INT AUTO_INCREMENT NOT NULL, nom VARCHAR(50) NOT NULL, prenom VARCHAR(50) NOT NULL, email VARCHAR(50) NOT NULL, mdp VARCHAR(255) NOT NULL, telephone VARCHAR(20) NOT NULL, adresse VARCHAR(100) NOT NULL, date_naissance DATE NOT NULL, photo LONGBLOB DEFAULT NULL, pseudo VARCHAR(50) NOT NULL, UNIQUE INDEX UNIQ_1D1C63B3E7927C74 (email), UNIQUE INDEX UNIQ_1D1C63B386CC499D (pseudo), PRIMARY KEY(utilisateur_id)) DEFAULT CHARACTER SET utf8mb4');
        ...
        $this->addSql('CREATE TABLE voiture (voiture_id INT AUTO_INCREMENT NOT NULL, modele VARCHAR(50) NOT NULL, immatriculation VARCHAR(20) NOT NULL, energie VARCHAR(255) NOT NULL, couleur VARCHAR(30) NOT NULL, date_premiere_immatriculation DATE NOT NULL, nb_places INT NOT NULL, utilisateur_id INT NOT NULL, marque_id INT NOT NULL, UNIQUE INDEX UNIQ_E9E2810FBE73422E (immatriculation), INDEX IDX_E9E2810FFB88E14F (utilisateur_id), INDEX IDX_E9E2810F4827B9B2 (marque_id), PRIMARY KEY(voiture_id)) DEFAULT CHARACTER SET utf8mb4');
        ...
        $this->addSql('ALTER TABLE suspension ADD CONSTRAINT FK_82AF05001B65292 FOREIGN KEY (employe_id) REFERENCES employe (employe_id)');
        ...
        $this->addSql('ALTER TABLE utilisateur_covoiturage ADD CONSTRAINT FK_DC21931AFB88E14F FOREIGN KEY (utilisateur_id) REFERENCES utilisateur (utilisateur_id)');
        $this->addSql('ALTER TABLE utilisateur_covoiturage ADD CONSTRAINT FK_DC21931A62671590 FOREIGN KEY (covoiturage_id) REFERENCES covoiturage (covoiturage_id)');
